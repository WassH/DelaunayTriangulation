...
Definition point2R1 (p: point) : rat :=
  p (Ordinal zero<1) (Ordinal(zero<2)).


Definition point2R2 (p: point) : rat :=
...
Definition point2namefun : point -> pointmap -> option P :=
 fun (p1 : point) (pm : pointmap) => 
 let S := [fset i in P | (pm i ==p1)]%fset in
...
(* graph est une fonction qui à un nom de triangle associe la liste des noms 
de triangles qui lui sont adjacents *)
...
Definition adjT (t1: T) (t2: T) (tm : trianglemap) : bool :=
  if [exists (i1 : 'I_3| true),
       [exists (i2: 'I_3 | true), 
          (triangle2points t1 tm i1 == triangle2points t2 tm i2) && 
     [exists (i3 : 'I_3|true), 
       [exists (i4 : 'I_3 | true), 
          (triangle2points t1 tm i3 == triangle2points t2 tm i4) 
               && (i1!=i3) && (i2!=i4)]]]]
                   then true
  else false.
...
(* predicat d'egalité entre 2 triangle basé sur perm_eq (doc à trouver dans 
  seq) *)
(* same_triangle t1 t2 dit si t1 et t2 sont des noms qui font référence au
   même triangle *)
...
(* unhookT t va retirer le triangle t de la tmap et va faire que g t = nil 
  et que les triangles qui étaient attachés à t1 perdent cette attache *)
...
let g' : T -> seq T := fun i : T => 
  if g t1 is t'::t''::t'''::s then
      if (~~(same_triangle i t1)) && (~~(same_triangle i t')) 
          && (~~(same_triangle i t'')) && (~~(same_triangle i t''')) then g i
      ...
      if (~~(same_triangle i t1)) && (~~(same_triangle i t'))
          && (~~(same_triangle i t''))then g i 
      else if same_triangle i t1 then nil
      else if same_triangle i t' then remove t1 (g t')
      else remove t1 (g t'')
  else if g t1 is t'::nil then
      if (~~(same_triangle i t1)) 
          && (~~(same_triangle i t'))then g i 
      else if same_triangle i t1 then nil
      else remove t1 (g t')
  else
...
(* Le rajout du nouveau triangle dans le graph sera fait au cas par cas dans
   les fonctions le nécessitant *)
Definition attachT (t1: triangle) (tm : trianglemap) (pm : pointmap)  := 
  if  point2namefun (t1 (Ordinal(zero<3))) pm is Some i1 then let i1' := i1 in
      if  point2namefun (t1 (Ordinal(un<3))) pm is Some i2 then
               let i2' := i2 in
          if point2namefun (t1 (Ordinal(deux<3))) pm is Some i3 then 
               let i3' := i3 in
                let tm' :=[ffun i :T => if i != [ffun x : 'I_3 =>
                                                     if x==0 then i1'
                                                     else if x==1 then i2'
                                                     else i3']  then tm i
                                        else t1] in Some tm'
...
(* Fonctions inTriangle, inHull, add_point_triangle, add_point_out, 
...
(* La fonction inTriangle va dire si un point donné est à l'intérieur d'un
   triangle donné *)
Definition inTriangle (p:point) (t:T) (tm : trianglemap) : bool :=

  let detM1 := leftpoint p (triangle2points t tm (Ordinal(zero<3))) 
                              (triangle2points t tm (Ordinal(un<3))) in
  let detM2 := leftpoint p (triangle2points t tm (Ordinal(un<3))) 
                              (triangle2points t tm (Ordinal(deux<3))) in
  let detM3 := leftpoint p (triangle2points t tm (Ordinal(deux<3)))
                           (triangle2points t tm (Ordinal(zero<3))) in

    if (detM1*detM2>0) && (detM2*detM3>0)  then true
    else false.


(* La fonction inHull va dire si un point donné est à l'intérieur de 
   l'enveloppe convexe dans ce cas elle retourne true ou sinon elle 
   retourne false. Elle est construite grâce à inTriangle*)
Definition inHull (p:point) (tmap : trianglemap) : bool := 
  [exists (i: T| true), inTriangle p i tmap].


(* La fonction ofindtriangle va (si c'est le cas) trouver et retourner un 
   Option T, car soit le point est dans l'enveloppe convexe est alors j'ai 
   un Some(i), i étant le nom du triangle et la preuve qu'il est dans tmap, 
   soit j'ai un None car il est hors enveloppe convexe. *)
(* Cette fonction peut être utilisée pour dire si un point est dans
   l'enveloppe convexe alors elle retourne Some u et si il n'y est pas alors
   elle retourne None *)
Definition ofindtriangle (p:point) (tm : trianglemap)  : option (T) := 
  [pick i: T| inTriangle p i tm].


(* La fonction findtriangle va prendre un argument de plus que ofindtriangle 
   une preuve inHull p ptm, que le point est dans l'enveloppe convexe et va 
   savoir retourner un domf tm. *)
...
(* Fonction qui va supprimer le triangle extérieur et qui va rajouter les 3
...
let t1 := [pick x | x \in [fset x | x in (g1 t) & [exists (i:'I_3|true),
                 (tm4 x (addOrd3 i (Ordinal(un<3))) ==tm4 t (Ordinal(zero<3)))
                      && (tm4 x i==tm4 t (Ordinal(un<3)))]]%fset] in
let t2 := [pick x | x \in [fset x | x in (g1 t) & [exists (i:'I_3|true), 
                 (tm4 x (addOrd3 i (Ordinal(un<3))) ==tm4 t (Ordinal(un<3))) 
                      && (tm4 x i==tm4 t (Ordinal(deux<3)))]]%fset] in
let t3 := [pick x | x \in [fset x | x in (g1 t) & [exists (i:'I_3|true), 
                 (tm4 x (addOrd3 i (Ordinal(un<3))) ==tm4 t (Ordinal(deux<3)))
                      && (tm4 x i==tm4 t (Ordinal(zero<3)))]]%fset] in

let g' : T -> seq T := fun i :T => 
        if t1 is Some (t1') then 
             if same_triangle i t1' then (g1 t1') ++ [::nomtri1]
             else g1 i
        else if t2 is Some (t2') then 
             if same_triangle i t2' then (g1 t2') ++ [::nomtri2]
             else g1 i
        else if t3 is Some (t3') then 
             if same_triangle i t3' then (g1 t3') ++ [::nomtri3]
             else g1 i
        else if same_triangle i nomtri1 then 
             if t1 is Some (t1') then [::t1']++[::nomtri2]++[::nomtri3]
             else [::nomtri2]++[::nomtri3]
        else if same_triangle i nomtri2 then 
             if t2 is Some (t2') then [::t2']++[::nomtri1]++[::nomtri3]
             else [::nomtri1]++[::nomtri3]
        else if same_triangle i nomtri3 then 
             if t3 is Some (t3') then [::t3']++[::nomtri1]++[::nomtri2]
...
(* add_point_triangle renvoie une option d'un nouveau graph et d'une nouvelle
   trianglemap *)
...
Let detfun := fun (nomp1 : P) (nomp2:P) (p:point) (pm : pointmap) => 
    ...
    let M2 := \col_(j < 3) if j==0 then point2R2 (pm nomp1)
                           else if j==1 then point2R2 (pm nomp2)
                           else point2R2 p in
...
Definition add_point_out  (p:point) (tm: trianglemap) (g:graph) 
                              (pm : pointmap) := 
  let hullmap2 := [ffun nomp : P => if (inHull (pm nomp) tm)==false 
                                        then (hullmap1 nomp) 
                                    else (nompnull,nompnull)] in
  let hull := [fset nomp|nomp in P & hullmap2 nomp!=(nompnull,nompnull)]%fset
       in
  (* badHull est l'ensemble des noms de points sur l'enveloppe convexe tel 
     que ce point et son successeur forment une arête à enlever *)
...
(* Fonction add_point globale, qui va faire appel selon les cas à 
   add_point_triangle ou add_point_out *)
(* Definition add_point (p:point) (tm:trianglemap) (g:graph) (pm : pointmap) 
                            := 
  if (inHull p tm) then 
                    let bool1 := inHull p tm in
                    let preuve := is_true(bool1) in
                    let t1 :=@findtriangle p tm preuve in
                      if add_point_triangle p t1 tm g pm is 
                            Some (g'', tm'') then 
                      let g':= g'' in 
                      let tm' := tm'' in (g',tm')
                      else (g, tm)
...
Definition change_ord (T : choiceType)(ss : {fset T}) (h : #|{:ss}| == 2) 
                    (x : 'I_2) : 'I_(#|{:ss}|).
...
let p2t2:= tm (val x) (Ordinal(deux<3)) in
      ...
      else if ~~((p0t1 == p0t2) ||(p0t1 == p1t2) ||(p0t1 == p2t2)) && 
        ~~((p0t2 == p0t1) ||(p0t2 == p1t1) ||(p0t2 == p2t1))
        then let ptext1 := p0t1 in let ptext2 := p0t2 in
            Some(ptext1, ptext2, val x, t1)
      else if ~~((p0t1 == p0t2) ||(p0t1 == p1t2) ||(p0t1 == p2t2)) && 
        ~~((p1t2 == p0t1) ||(p1t2 == p1t1) ||(p1t2 == p2t1))
        then let ptext1 := p0t1 in let ptext2 := p1t2 in
            Some(ptext1, ptext2, val x, t1)
      else if ~~((p0t1 == p0t2) ||(p0t1 == p1t2) ||(p0t1 == p2t2)) && 
        ~~((p2t2 == p0t1) ||(p2t2 == p1t1) ||(p2t2 == p2t1))
        then let ptext1 := p0t1 in let ptext2 := p2t2 in
            Some(ptext1, ptext2, val x, t1)

      else if ~~((p1t1 == p0t2) ||(p1t1 == p1t2) ||(p1t1 == p2t2)) &&
        ~~((p0t2 == p0t1) ||(p0t2 == p1t1) ||(p0t2 == p2t1))
        then let ptext1 := p1t1 in let ptext2 := p0t2 in
            Some(ptext1, ptext2, val x, t1)
      else if ~~((p1t1 == p0t2) ||(p1t1 == p1t2) ||(p1t1 == p2t2)) &&
        ~~((p1t2 == p0t1) ||(p1t2 == p1t1) ||(p1t2 == p2t1))
        then let ptext1 := p1t1 in let ptext2 := p1t2 in
            Some(ptext1, ptext2, val x, t1)
      else if ~~((p1t1 == p0t2) ||(p1t1 == p1t2) ||(p1t1 == p2t2)) &&
        ~~((p2t2 == p0t1) ||(p2t2 == p1t1) ||(p2t2 == p2t1))
        then let ptext1 := p1t1 in let ptext2 := p2t2 in
            Some(ptext1, ptext2, val x, t1)

      else if ~~((p2t1 == p0t2) ||(p2t1 == p1t2) ||(p2t1 == p2t2)) &&
        ~~((p0t2 == p0t1) ||(p0t2 == p1t1) ||(p0t2 == p2t1))
        then let ptext1 := p2t1 in let ptext2 := p0t2 in
            Some(ptext1, ptext2, val x, t1)
      else if ~~((p2t1 == p0t2) ||(p2t1 == p1t2) ||(p2t1 == p2t2)) &&
        ~~((p1t2 == p0t1) ||(p1t2 == p1t1) ||(p1t2 == p2t1))
        then let ptext1 := p2t1 in let ptext2 := p1t2 in
            Some(ptext1, ptext2, val x, t1)
      else let ptext1 := p2t1 in let ptext2 := p2t2 in
            Some(ptext1, ptext2, val x, t1)
...
(* RMQ : Si ce point n'est pas un des 3 sommets d'un des 2 triangles alors
   la fonction retourne Ordinal(deux<3) *)
Definition point2indext1t2 (p:point) (t1:T) (t2:T) (tm : trianglemap) 
                : 'I_3 :=
let p0t1:= tm t1 (Ordinal(zero<3)) in
let p1t1:= tm t1 (Ordinal(un<3)) in
let p2t1:= tm t1 (Ordinal(deux<3)) in
...
Definition flip (tm: trianglemap)  (ptext1 : point) (ptext2 : point) (t1:T) 
                    (t2 :T) (g:graph) (pm: pointmap) :=
let p0t1:= tm t1 (Ordinal(zero<3)) in
let p1t1:= tm t1 (Ordinal(un<3)) in
let p2t1:= tm t1 (Ordinal(deux<3)) in
let p0t2:= tm t2 (Ordinal(zero<3)) in
...
in let triangle2 := fun x:'I_3 => 
   ...
   else tm t1 (addOrd3 indexptext2 (Ordinal(deux<3)))
...
(* nomt1 est le nom du triangle passant par ptext1 et edjacent à l'autre 
   triangle *)
let nomt1 := if (ptext1 == p0t1) || (ptext1 == p1t1) || (ptext1 == p2t1)
                 then t1
             else t2 in

(* nomt2 est le nom du triangle passant par ptext2 et edjacent à l'autre
   triangle *)
let nomt2 := if (ptext2 == p0t2) || (ptext2 == p1t2) || (ptext2 == p2t2) 
                 then t2
             else t1 in

(* nomtri1 est le nom de triangle1 *)
let nomtri1 := [ffun x:'I_3 =>
      if x==0 then nomt1 indexptext1
      else if x==1 then nomt2 indexptext2
      else nomt2 (addOrd3 indexptext2 (Ordinal(un<3)))] in

(* nomtri2 est le nom de triangle2 *)
let nomtri2 := [ffun x:'I_3 =>
      if x==0 then nomt2 indexptext2
      else if x==1 then nomt1 indexptext1
      else nomt1 (addOrd3 indexptext1 (Ordinal(un<3)))]  in

(* p1 est l'un des 2 points adjacents à t1 et t2, c'est celui qui est dans 
   triangle1 *)
let p1 := tm3 nomt1 (addOrd3 indexptext1 (Ordinal(deux<3))) in
(* p2 est l'un des 2 points adjacents à t1 et t2, c'est celui qui est dans
   triangle2 *)
let p2 := tm3 nomt2 (addOrd3 indexptext2 (Ordinal(deux<3))) in


  let tbr := [pick x | x \in [fset x | x in (g3 nomt2) &
      [exists (i:'I_3|true), (tm5 x (addOrd3 i (Ordinal(un<3))) ==p2) 
            && (tm5 x i==ptext2)]]%fset] in
  let ttr := [pick x | x \in [fset x | x in (g3 nomt2) &
     [exists (i:'I_3|true), (tm5 x (addOrd3 i (Ordinal(un<3))) ==ptext2)
            && (tm5 x i==p1)]]%fset] in
  let ttl := [pick x | x \in [fset x | x in (g3 nomt1) &
     [exists (i:'I_3|true), (tm5 x (addOrd3 i (Ordinal(un<3))) ==ptext1)
            && (tm5 x i==ptext1)]]%fset] in
  let tbl := [pick x | x \in [fset x | x in (g3 nomt1) &
     [exists (i:'I_3|true), (tm5 x (addOrd3 i (Ordinal(un<3))) ==ptext1)
            && (tm5 x i==p2)]]%fset] in
    ...
    let g' : T -> seq T := fun i :T => 
      ...
      if tbr is Some (tbr') then if same_triangle i tbr' 
                                      then (g tbr') ++ [::nomtri2]
                                 else g i
      else if ttr is Some (ttr') then if same_triangle i ttr' 
                                        then g(ttr') ++ [::nomtri1]
                                       else g i
      else if ttl is Some (ttl') then if same_triangle i ttl' 
                                        then g(ttl') ++ [::nomtri1]
                                       else g i
      else if tbl is Some (tbl') then if same_triangle i tbl'
                                        then g(tbl') ++ [::nomtri2]
                                      else g i
      else if same_triangle i nomtri1 then if ttr is Some (ttr') 
                                       then if ttl is Some (ttl') 
                                          then [::nomtri2]++[::ttr']++[::ttl']
                                            else [::nomtri2]++[::ttr']
                                           else if ttl is Some (ttl') 
                                                  then [::nomtri2]++[::ttl']
                                                else [::nomtri2]
      else if same_triangle i nomtri2 then if tbr is Some (tbr') 
                                       then if tbl is Some (tbl') 
                                          then [::nomtri1]++[::tbr']++[::tbl']
                                            else [::nomtri1]++[::tbr']
                                           else if tbl is Some (tbl') 
                                                  then [::nomtri1]++[::tbl']
                                                else [::nomtri1]
      else g i in 
